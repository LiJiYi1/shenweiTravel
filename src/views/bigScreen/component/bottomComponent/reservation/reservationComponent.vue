...
let sum = trafficWay.reduce((cur, pre) => { return cur + pre.value}, 0)
let data = [];
let legendData:string[] = [];
var color=['#00ffff','#409B5C','#006ced','#ffe000','#ffa800','#ff5b00','#ff3000']
for (let i = 0; i < trafficWay.length; i++) {
    let name:string = trafficWay[i].name + '--' + (trafficWay[i].value / sum * 100 ).toFixed(1) + '%'
...
<style lang="less" scoped>
    .reservation{
        height: 240px;
        width: 350px;
        background: url('@/assets/bigScreen/images/dataScreen-main-rb.png') ;
